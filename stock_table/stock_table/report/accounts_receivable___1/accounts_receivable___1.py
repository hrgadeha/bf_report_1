...
class ReceivablePayableReport(object):
	...
	def get_data(self, party_naming_by, args):
		if not self.filters.get("customer"):
			msgprint(_("Please select Customer"), raise_exception=1)
	...
	def prepare_row(self, party_naming_by, args, gle, outstanding_amount, credit_note_amount,
		...
		if args.get("party_type") == "Customer":
			row += [get_po_no(gle.voucher_no),self.voucher_details.get(gle.voucher_no, {}).get("delivery_note"),self.get_territory(gle.party), self.get_customer_group(gle.party),
...
def get_po_no(s_id):
	lpo_no=frappe.db.get_value("Sales Invoice",s_id,"lpo_number")
	if lpo_no:
		return lpo_no
	else:
		return ''
